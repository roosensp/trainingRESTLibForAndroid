...
public class RequestServerRest extends AsyncTask<String, Void, String>{
    ...
    private void deleteRequest( )
	{
		try
		{
			 ...
			 HttpDelete request = new HttpDelete(this.url + this.rechercheEndUrl);
			 Log.v("test" , " METHODE DELETE , URL => " +this.url + this.rechercheEndUrl ) ;
	...
	private void postRequest()
	{
		try
		{
			...
			Log.v("test" , "URL => " + this.url + this.rechercheEndUrl) ;
			request.addHeader("Content-type", this.typeAnwswer);
			request.setEntity(entity) ;
			HttpResponse response = client.execute(request);
			int code = response.getStatusLine().getStatusCode();
    ...
    protected void onPostExecute(String result) { 
		...
		if(this.typeConnection == "GET")
		{
			...
			{
				classWhoAsk.receiveResponse(this.response, this.rechercheEndUrl) ;
			}
	...
	public Boolean getResponseIsArray() {
